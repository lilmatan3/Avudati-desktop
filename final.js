...
function populateFinalPaystub() {
  ...
  // ➕ תוספות/הורדות מהעמוד deductions-settings
  ...
  const creditPoints = tax?.creditPoints || 2.25;
  ...
  const pension = (tax?.pension || 0) / 100 * grossForPension;
  const hishtalmut = (tax?.hishtalmut || 0) / 100 * grossForTax;
  ...
  const employerSeverance = (tax?.severance || 0) / 100 * grossForTax;
